<?php
	$this->assign('title', 'Create New Contact');
	$this->set('wunder_title', 'Create New Contact');
?>
<div>
	...
	<div class="form-section">
		...
		<div id="is-contact-new" data-check="<?php echo $contact->isNew(); ?>"></div>
		...
		<div class="company-inputs">
			...
			<div class="field is-horizontal">
			  ...
			  <div class="field-body">
		  	  	<?php echo $this->Form->control('company_incorporated_in', ['class' => 'select is-fullwidth is-small incorporated-in', 'empty' => true, 'label' => false, 'options' => $states]); ?>
			  </div>
			</div>

			<div id="domestic-foreign" class="field is-horizontal" style="<?= (!empty($contact->company_domestic_foreign)) ? '' : 'display:none' ?>">
			  <div class="field-label is-small">
		...
		<?php if (isset($contact->contact_addresses)) { 
			foreach ($contact->contact_addresses as $key => $address) {
		...
		<div class="foreign-toggle" style="<?= ($contact->company_domestic_foreign == 'foreign') ? 'display:none' : '' ?>">
			...
			<?php if (isset($contact->contact_phone_numbers)) { 
				foreach ($contact->contact_phone_numbers as $key => $number) {
			...
			<?php if (isset($contact->contact_emails)) {
				foreach ($contact->contact_emails as $key => $email) {
			...
			<?php if (isset($contact->contact_websites)) {
				foreach ($contact->contact_websites as $key => $website) {
			...
			<?php if (isset($contact->contact_dbas)) {
		...
		<div class="table-container" style="margin-bottom:0px">
			<table class="table is-narrow" data-table="contactRelationships">
				<tbody>
					<?php if (isset($contact->contact_relationships)) { foreach ($contact->contact_relationships as $key => $relationship) {
						echo $this->element('relationship', ['key' => $key, 'contact' => $contact, 'relationship' => $relationship]);
					}} ?>
					<tr>
						<td colspan="4" style="min-width: 300px;"></td>
		...
		<div class="table-container" style="margin-bottom:0px">
			<table class="table is-narrow" data-table="targetRelationships" style="margin-bottom:0px">
				<tbody>
					<?php if (isset($contact->target_relationships)) { foreach ($contact->target_relationships as $key => $relationship) {
